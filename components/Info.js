...
const Info = ({ swal }) => {
  ...
  const handleClick = () => {
    const statsss = renderToString(states);

    const message = `${statsss}`;
  ...
  return (
    <div className="absolute bottom-4 right-4 flex items-center justify-center gap-1">
      <span onClick={handleClick} className="text-gray-600 text-sm">
        info
      </span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
...
export default withSwal(({ swal }, ref) => <Info swal={swal} />);
